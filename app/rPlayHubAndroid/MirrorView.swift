...
final class MirrorView: NSView, NSMenuItemValidation {
    ...
    /// A pre-Oreo device driven by the legacy agent; input goes to it when set.
    var legacy: LegacySession?
    /// Input is live once either path is up.
    private var inputLive: Bool { control != nil || emulator != nil || legacy != nil }
    ...
    private func sendMotion(_ point: CGPoint, action: Int32) {
        ...
        if let legacy {
            // 1 down, 2 move, 0 up — the legacy agent's touch phases.
            let phase = action == MotionAction.down ? 1 : (action == MotionAction.up ? 0 : 2)
            legacy.touch(x: Int(x), y: Int(y), phase: phase)
            return
        }
        if let emulator {
