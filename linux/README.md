...
- **Phone in its own window** (Open Screen in New Window, `--pop-out`): a bare, rounded window showing nothing but the phone's screen, which the main window's stage then leaves to it ("Bring Back" returns it). When the pointer comes near, the window grows around the unchanged picture to append a title bar with traffic lights and the control strip, like the Mac's; it goes bare again when the pointer leaves (`RPLAYHUB_POPOUT_FIXED=1` keeps the window still and fits the bars inside instead). Rotating the phone turns the window with it. The corners are real on X11 (the window has an alpha channel; the compositor shows the desktop through them), square on Wayland. Desktop Mode and app windows are the same kind of window.
