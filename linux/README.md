...
- **⋮ menu** at the top right with the Mac client's menus as sub-entries: **Device** (Start/Stop Screen Mirroring Ctrl+M, Follow Device Rotation, Turn Screen Off While Mirroring, Synchronize Clipboard, Forward Audio, Pause Display, Desktop Mode Ctrl+D, Open Front App on Virtual Display Ctrl+Shift+F, Close Virtual Displays, Show Files in File Manager Ctrl+Shift+E, Refresh Devices Ctrl+R), **View** (Open Screen in New Window Ctrl+N, Pin Window on Top Ctrl+P, View Screen in 3D Ctrl+3, Set Facing Me R), **Window** (Minimize, Zoom, Bring All to Front), **Help** (Help F1, GitHub links), then About and Quit Ctrl+Q. The control strip and the right-click menus carry the per-screen actions (keys, rotate, screenshot, record), as on the Mac.
- **Inspector groups** like the Mac's: the three title-bar icons are Settings (developer toggles on the device: show taps, pointer location, refresh rate, stay awake, animation scales), Logs (Logcat, Crashes from the crash buffer or DropBox) and Info (Info, Apps, Files), each with its own text tabs.
- **Focus colours** as on the Mac: the sidebar's selected row is accent blue only while the sidebar is the focused pane, gray otherwise; the View Screen button lights only while the stage is focused and the window is active.
- **Phone in its own window** (Open Screen in New Window, `--pop-out`): a bare, rounded window showing nothing but the phone's screen, which the main window's stage then leaves to it ("Bring Back" returns it). When the pointer comes near, a title bar with traffic lights and the control strip fade in around the phone, like the Mac's, and fade out when the pointer leaves. The window itself never moves or resizes: it is created with room for the bars, transparent outside the phone, and an X input shape lets clicks in the transparent margins fall through to whatever is behind. Rotating the phone turns the window with it. The corners are real on X11 (the window has an alpha channel; the compositor shows the desktop through them), square on Wayland. Desktop Mode and app windows are the same kind of window.
- **Foldables** (Pixel Fold family): the agent announces the device's posture (CLOSED, HALF_OPENED, OPENED) and streams the hinge angle and both halves' gyroscopes on its sensor channel; the client shows them in the Info tab (Fold, Hinge, Gyro A/B), follows the panel swap when the phone closes (the stream moves to the outer display) and draws the fold as it happens: the inner picture as two halves hinged at the crease, turned by the live hinge angle, then the outer panel once closed. `RPLAYHUB_FAKE_HINGE=1` sweeps the angle for development without a foldable; `RPLAYHUB_SENSOR_DEBUG=1` traces hinge packet arrival.
- **Duo twin for foldables**: in the 3D view a foldable is two slabs hinged at the crease. The held half takes the sensor pose, the other turns by the live hinge angle; the inner picture is split across the two inner faces, the outer panel's picture (or the back artwork) sits on the outer faces. Three render modes, keys 1/2/3: hard cut (what Android draws), locked (a content plane fixed in the world, sampled along the camera ray, the ground truth of the brief), stylized (an alpha ramp and a seam band on the moving half). `RPLAYHUB_TWIN_MODE=0|1|2` picks the starting mode; `RPLAYHUB_FAKE_HINGE=<deg>` holds a fixed angle.
